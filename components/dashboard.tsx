...
function ConversationCard({ conversation: c, savingId, onStatus }: { conversation: Conversation; savingId: string | null; onStatus: (c: Conversation, s: ConversationStatus) => void }) {
  return <article className="p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-bold">{c.contact_name ?? "Tanpa nama"}</p><p className="mt-1 flex items-center gap-1.5 text-xs font-medium text-slate-500"><Phone className="size-3.5" />{c.contact_phone ?? "Nomor belum tersedia"}</p></div><Badge className={priorityClass[c.priority]}>{labels.priority[c.priority]}</Badge></div><p className="mt-3 text-sm leading-6 text-slate-700">{c.summary}</p><div className="mt-3 flex flex-wrap gap-2"><Badge className="bg-teal-50 text-teal-700">{labels.stage[c.stage]}</Badge><Badge className="bg-slate-100 text-slate-600">{labels.blocker[c.blocker]}</Badge><Badge className={statusClass[c.status]}>{labels.status[c.status]}</Badge></div><div className="mt-4 rounded-xl bg-slate-50 p-3"><p className="text-xs font-bold uppercase tracking-wide text-slate-400">Tindakan berikutnya</p><p className="mt-1 text-sm font-medium leading-6 text-slate-700">{c.next_action}</p></div><div className="mt-3 rounded-xl border border-slate-200 p-3"><p className="text-xs text-slate-500">{dateLabel(c.last_message_at)}</p><Badge className="mt-2 bg-slate-100 text-slate-600">Dari: {labels.sender[c.last_message_sender]}</Badge></div><div className="mt-4"><StatusSelect conversation={c} onChange={(status) => onStatus(c, status)} disabled={savingId === c.id} /></div></article>;
}
