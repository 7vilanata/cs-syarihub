...
import { ArrowUpRight, CheckCircle2, ChevronDown, CircleDashed, FilterX, LogOut, MessageCircleMore, Search, Sparkles, UsersRound } from "lucide-react";
...
export function Dashboard({ initialConversations }: { initialConversations: Conversation[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    return conversations.filter((item) => {
      ...
      return (!term || item.conversation_id.toLowerCase().includes(term) || item.contact_name?.toLowerCase().includes(term)) &&
    ...
    <main className="mx-auto max-w-[1600px] px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
      ...
      <section className="mt-6 rounded-2xl border border-slate-200 bg-white shadow-[0_1px_3px_rgb(15_23_42/5%)]">
        <div className="border-b border-slate-200 p-4 sm:p-5"><div className="flex flex-col gap-3 xl:flex-row xl:items-center">
          <div className="relative min-w-64 flex-1"><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-slate-400" /><Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Cari nama atau ID conversation" className="h-10 pl-9" /></div>
...
function ConversationTable({ conversations, savingId, onStatus }: { conversations: Conversation[]; savingId: string | null; onStatus: (c: Conversation, s: ConversationStatus) => void }) {
  return <table className="w-full min-w-[1220px] border-collapse text-left"><thead><tr className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500"><th className="px-5 py-3 font-bold">Calon user</th><th className="px-4 py-3 font-bold">Ringkasan AI</th><th className="px-4 py-3 font-bold">Stage & blocker</th><th className="px-4 py-3 font-bold">Prioritas</th><th className="px-4 py-3 font-bold">Tindakan berikutnya</th><th className="px-4 py-3 font-bold">Status</th><th className="px-5 py-3" /></tr></thead><tbody className="divide-y divide-slate-100">{conversations.map((c) => <tr key={c.id} className="align-top transition hover:bg-slate-50/70"><td className="w-[180px] px-5 py-4"><p className="font-bold text-slate-900">{c.contact_name ?? "Tanpa nama"}</p><p className="mt-2 text-xs text-slate-500">{dateLabel(c.last_message_at)}</p><Badge className="mt-1 bg-slate-100 text-slate-600">Terakhir: {labels.sender[c.last_message_sender]}</Badge></td><td className="max-w-[280px] px-4 py-4"><p className="text-sm leading-6 text-slate-700">{c.summary}</p></td><td className="w-[170px] px-4 py-4"><Badge className="bg-teal-50 text-teal-700">{labels.stage[c.stage]}</Badge><p className="mt-2 text-xs text-slate-500">Hambatan: <span className="font-semibold text-slate-700">{labels.blocker[c.blocker]}</span></p></td><td className="w-[105px] px-4 py-4"><Badge className={priorityClass[c.priority]}>{labels.priority[c.priority]}</Badge></td><td className="max-w-[260px] px-4 py-4"><p className="text-sm font-medium leading-6 text-slate-700">{c.next_action}</p></td><td className="w-[175px] px-4 py-4"><StatusSelect conversation={c} onChange={(status) => onStatus(c, status)} disabled={savingId === c.id} /></td><td className="w-[145px] px-5 py-4 text-right"><Button asChild variant="outline" size="sm"><a href={c.conversation_url} target="_blank" rel="noopener noreferrer">Buka chat <ArrowUpRight className="size-4" /></a></Button></td></tr>)}</tbody></table>;
}
...
function ConversationCard({ conversation: c, savingId, onStatus }: { conversation: Conversation; savingId: string | null; onStatus: (c: Conversation, s: ConversationStatus) => void }) {
  return <article className="p-4"><div className="flex items-start justify-between gap-3"><p className="font-bold">{c.contact_name ?? "Tanpa nama"}</p><Badge className={priorityClass[c.priority]}>{labels.priority[c.priority]}</Badge></div><p className="mt-3 text-sm leading-6 text-slate-700">{c.summary}</p><div className="mt-3 flex flex-wrap gap-2"><Badge className="bg-teal-50 text-teal-700">{labels.stage[c.stage]}</Badge><Badge className="bg-slate-100 text-slate-600">{labels.blocker[c.blocker]}</Badge><Badge className={statusClass[c.status]}>{labels.status[c.status]}</Badge></div><div className="mt-4 rounded-xl bg-slate-50 p-3"><p className="text-xs font-bold uppercase tracking-wide text-slate-400">Tindakan berikutnya</p><p className="mt-1 text-sm font-medium leading-6 text-slate-700">{c.next_action}</p></div><p className="mt-3 text-xs text-slate-500">Pesan {labels.sender[c.last_message_sender].toLowerCase()} • {dateLabel(c.last_message_at)}</p><div className="mt-4 flex flex-col gap-2 sm:flex-row"><StatusSelect conversation={c} onChange={(status) => onStatus(c, status)} disabled={savingId === c.id} /><Button asChild variant="outline" className="sm:ml-auto"><a href={c.conversation_url} target="_blank" rel="noopener noreferrer">Buka conversation <ArrowUpRight className="size-4" /></a></Button></div></article>;
}
